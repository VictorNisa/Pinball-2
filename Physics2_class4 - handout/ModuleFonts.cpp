...
int ModuleFonts::Load(const char* texture_path, const char* characters, uint rows)
{
	...
	fonts[id].len = strlen(characters); // Length
	...
	fonts[id].char_w /= fonts[id].len / fonts[id].rows;
...
void ModuleFonts::BlitText(int x, int y, int font_id, const char* text) const
{
	...
	for (uint i = 0; i < len; ++i)
	{
		...
		for (uint z = 0; z < font->len; ++z) {
			if (font->table[z] == text[i]) {
				if (font->rows == 1) {
					rect.x = z * font->char_w;
					rect.y = 0;
				}
				else if (font->rows == 2) {
					if (z > font->row_chars) {
						rect.x = (z - font->row_chars) * font->char_w;
						rect.y = font->char_h;
					}
					else {
						rect.x = z * font->char_w;
				...
				App->renderer->Blit(font->graphic, x, y, &rect, 0.0f, false);
				x += font->char_w;
				break;
			}
		}
